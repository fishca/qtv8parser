...
CV8File::CV8File(const CV8File &src) : FileHeader(src.FileHeader), IsDataPacked(src.IsDataPacked)
{
...
// Кастомный streambuf, читающий из vector<char>
class vector_streambuf : public std::streambuf {
public:
    explicit vector_streambuf(const std::vector<char>& data) {
        // Устанавливаем указатели на начало и конец буфера (только для чтения)
        char* begin = const_cast<char*>(data.data());
        char* end = begin + data.size();
        setg(begin, begin, end);  // get area pointers
    }
};
...
public:
	explicit vector_data_source_t(vector<char> data) :
        ...
        , __streambuf(__data)      // инициализируем streambuf ссылкой на данные
        , __stream(&__streambuf)   // istream использует наш streambuf	{}
    {}
...
private:
	vector<char> __data;
    vector_streambuf __streambuf;  // должен жить дольше, чем __stream
...
{
	return filter.empty() || find(filter.begin(), filter.end(), name) != filter.end();
}
...
static bool is_dot_file(const std::filesystem::path &path)
{
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    // Генерация уникального имени файла без Boost
                    auto now = std::chrono::steady_clock::now().time_since_epoch().count();
                    auto rnd = std::random_device{}();
                    std::string unique_name = "tmp_" + std::to_string(now) + "_" + std::to_string(rnd);

                    std::filesystem::path tmp_file_path = std::filesystem::temp_directory_path() / unique_name;
...
int CV8File::GetData(vector<char> &data)
{
	...
	auto NeedDataBufferSize = format::file_header_t::Size() + format::block_header_t::Size() + MAX(format::elem_addr_t::Size() * ElemsNum, format::DEFAULT_PAGE_SIZE);
