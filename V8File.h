...
struct stBlockHeader
{
	...
	{
		return EOL_0D == 0x0d && EOL_0A == 0x0a && space1 == 0x20 && space2 == 0x20 && space3 == 0x20 && EOL2_0D == 0x0d && EOL2_0A == 0x0a;
	}
...
{
	std::copy(std::istreambuf_iterator<T>(in_file), std::istreambuf_iterator<T>(), std::ostreambuf_iterator<T>(out_file));
}
